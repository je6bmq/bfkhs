...
parseInstructions program = parseInstructionsImpl 0 program $ Right V.empty
  where
    ...
    parseInstructionsImpl column (c : program) insts =
      case c of
        ...
        _ -> case parseUnaryInstruction c of
          Right inst -> parseInstructionsImpl (column + 1) program $ (V.++) <$> insts <*> sequence (V.singleton $ Right inst)
          Left errFunc -> Left $ errFunc column
    ...
    parseLoopInstruction (c : cs) insts = parseLoopInstruction cs $ (V.++) <$> insts <*> sequence (V.singleton $ parseUnaryInstruction c)
